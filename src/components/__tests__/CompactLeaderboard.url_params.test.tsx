...
describe('CompactLeaderboard - URL Parameter Control', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mockNavigate.mockClear();
  });

  it('should set ?feed=global URL param when "Global" tab is clicked in sidebar', async () => {
    const user = userEvent.setup();

    renderWithAuth(<CompactLeaderboard />, mockUser, ['/?feed=following']);
  ...
  it('should set ?feed=following URL param when "Friends" tab is clicked in sidebar', async () => {
    const user = userEvent.setup();

    renderWithAuth(<CompactLeaderboard />, mockUser, ['/?feed=global']);
  ...
  it('should sync tab state with URL param in sidebar', async () => {
    ...
    await waitFor(() => {
      ...
      const globalTab = screen.getByText(/Global/i).closest('button');
      expect(globalTab).toHaveClass(/active|selected|bg-green-700/i);
    });
  });

  it('should default to global tab if no URL param is present in sidebar', async () => {
    renderWithAuth(<CompactLeaderboard />, mockUser, ['/']);
